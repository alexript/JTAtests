 ...
 */
public class BitronixConsole extends JFrame {

    protected static final SimpleDateFormat dateFormatter = new SimpleDateFormat("dd-MM-yyyy HH:mm:ss.SSS");
    private static final long serialVersionUID = -8810532940280791146L;
    ...
    public BitronixConsole(Configuration config) throws IOException {
        final Configuration configuration = config == null ? TransactionManagerServices.getConfiguration() : config;
        ...
        filterByGtridItem.addActionListener((ActionEvent e) -> {
        ...
        switchLogFilesItem.addActionListener((ActionEvent evt) -> {
            switchLogFiles(configuration);
        });

        countDuplicatedGtridsItem.addActionListener((ActionEvent evt) -> {
            countDuplicatedGtrids();
        });

        countByStatus.addActionListener((ActionEvent evt) -> {
            countByStatus();
        });

        bySequenceItem.addActionListener((ActionEvent evt) -> {
            findBySequence();
        });

        byGtridItem.addActionListener((ActionEvent evt) -> {
            findByGtrid();
        });
    ...
    private void countDuplicatedGtrids() {
        ...
        HashMap<Uid, java.util.List<TransactionLogRecord>> gtrids = new HashMap<>();
        HashMap<Uid, java.util.List<TransactionLogRecord>> redundantGtrids = new HashMap<>();
        ...
        for (int i = 0; i < transactionTableModel.getRowCount(); i++) {
            ...
            if (tlog.getStatus() == Status.STATUS_COMMITTING) {
                ...
                if (gtrids.containsKey(gtrid)) {
                    ...
                    java.util.List<TransactionLogRecord> tlogs = new ArrayList<>();
                    tlogs.add(tlog);
...
//        JOptionPane.showMessageDialog(this, redundantGtrids.size() + " duplicated GTRID", "Duplicated GTRID count", JOptionPane.INFORMATION_MESSAGE);
    ...
    private void findBySequence() {
        ...
        try {
            searchedSequence = Integer.parseInt(sequence);
